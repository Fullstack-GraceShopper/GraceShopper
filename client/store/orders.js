...
export const postToCart = (sockId, size, quantity) => async dispatch => {
  try {
    const {data: order} = await axios.post(`/api/orders/cart/add?sockId=${sockId}&size=${size}&quantity=${quantity}`);
